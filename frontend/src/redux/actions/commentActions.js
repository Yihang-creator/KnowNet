...
export const changeLike =
	(post, isCancel, userID, accessToken) => async (dispatch) => {
...
export const addComment =
	(postId, userId, commentText, accessToken) => async (dispatch) => {
		const newComment = {
			postId,
			userId,
			text: commentText,
	...
	async (dispatch) => {
		...
		try {
			...
			const response = await fetch(
				`/api/comments/replies/${commentId}/${userId}/${replyTo}`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
...
export const fetchComments = (postId, accessToken) => async (dispatch) => {
	try {
		const response = await fetch(
			`/api/comments/getCommentDetail?postId=${postId}`,
			{
				headers: {
					Authorization: 'Bearer ' + accessToken,
				},
...
export const fetchLikes =
	(postId, commentId, replyId, userId, accessToken) => async (dispatch) => {
		const params = {
			postId,
			commentId,
			replyId,
