...
import {Box, Button, CardContent, Dialog, DialogActions, Fab, Grid, IconButton, Typography, Zoom,} from '@mui/material';
import {FavoriteBorder, Message, Notes, PersonAddAlt1, PersonOff} from '@mui/icons-material';
...
const UserInfoPage = ({name, email}) => {
    ...
    useEffect(() => {
        const ifFollowing = (users || []).some(i => i.userId === currentUserInfo.userId)
    ...
    const handleFollow = () => { // follow/unfollow
        let operation = selected ? "unfollow" : "follow";
        dispatch(follow(currentUserInfo.userId, userInfo.userId, operation, 'followers', oktaAuth.getAccessToken(), () => {
            ...
            <Box sx={{flexGrow: 1}}>
                <div className="flex flex-col gap-4 p-2">
                    ...
                    >
                        ...
                        {/*  <Dropdown />*/}
                        {/*</div>*/}
                        <Grid className="flex p-5 pt-7 justify-between">
                            <Grid className="flex">
                                <div className="relative h-36 w-36 justify-center mt-4 rounded-full bg-white">
                                    {
                                        isSelf ? <div className="absolute bottom-0 right-4 z-10">
                                            <label
                                ...
                                <Box className="items-center flex pl-2">
                                    <CardContent sx={{flex: '1 0 auto'}}>
                                        <Typography component="div" variant="h4" color="#fff"
                        ...
                        <Grid className="flex justify-between p-2 text-white">
                            <Grid className="flex gap-2 items-center w-full justify-between">
                                <Grid className="flex ">
                                    <PopupButton type="followings" token={oktaAuth.getAccessToken()} isSelf={isSelf}
                                                 userInfo={userInfo}/>
                                    <PopupButton type="followers" token={oktaAuth.getAccessToken()} isSelf={isSelf}
                                                 userInfo={userInfo} selected={selected}/>
                    ...
                    <Grid sx={{flexDirection: 'column', background: '#515f72'}}
                        ...
                        <Grid className="flex justify-center gap-4 rounded-md p-2">
                            ...
                            >
                                <Notes sx={{marginRight: '5px'}}/>Posts
                            </ColorButton>
                            <ColorButton
                                variant="contained"
                                sx={{margin: '0 15px'}}
                            ...
                            >
                                <FavoriteBorder sx={{marginRight: '5px'}}/>Liked
                            </ColorButton>
                        </Grid>
