...
import { Button, TextField, Divider,Typography, Box, Stack, Grid, Avatar } from '@mui/material';
...
const JoinVideoRoom = (props) => {
    ...
    const [url, setUrl] = useState(props.url || 'https://www.youtube.com/watch?v=QZw-rgaQVfI');
    ...
    return (
        <Box minHeight="95vh">
          <Stack direction={{ xs: "column", md: "row" }} spacing={4}>
            <Box flex={2}>
              <Box display='flex' flexDirection='column' sx={{ width: "100%", position: "sticky", top: "86px" }}>
                <ReactPlayer
                ref={ref}
                url={url}
                ...
                {queue_control}
                <Box display="flex" flexDirection="column" overflow='auto' sx={{maxHeight: "90vh"}}>
                    <VideoQueue
                    changeVideo={(vid) => {
            ...
            <Box flexDirection='column' overflow='auto' sx={{maxHeight: "80vh", flex: 1, display: "flex"}} >
                ...
                <div ref={chatRef} style={{overflowY: "auto" }} color='primary'>
                    ...
                    return (
                        <Box key={idx} display="flex" justifyContent={msg.user === username ? "flex-end" : "flex-start"} mb={2}>
                          <Grid container spacing={2} alignItems="flex-end" direction={msg.user === username ? "row-reverse" : "row"}>
                            <Grid item>
                            ...
                            <Grid item xs zeroMinWidth>
                              ...
                              <Typography variant="body2" align={msg.user === username ? "right" : "left"}>{msg.time}</Typography>
                            </Grid>
                          </Grid>
                        </Box>
                      );
