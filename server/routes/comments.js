...
router.post(
  '/replies/:commentId/:userId/:replyTo',
  async function (req, res, next) {
...
router.get('/getCommentDetail', async function (req, res, next) {
  ...
  try {
    const relatedComments = await Comment.find({ postId: postId })
    ...
    const newComments = relatedComments
      .map((comment) => {
        ...
        if (postId === comment1.postId) {
          const newComment = {
            ...comment1,
            user:
              allUsers.filter((user) => user.userId === comment1.userId)[0] ||
              {},
            replies: (comment1.replies || []).map((reply) => ({
              ...reply,
              user:
                allUsers.filter((user) => user.userId === reply.userId)[0] ||
                {},
              replyToUser: reply.isSecLevelComment
                ? allUsers.filter((user) => user.userId === reply.replyTo)[0] ||
                  {}
                : {},
            })),
          };
